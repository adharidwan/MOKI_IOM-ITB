...
with numbered as (
  select
    id,
    row_number() over (order by created_at asc, id asc) as next_display_id
  from public.content_recordings
)
update public.content_recordings cr
set display_id = numbered.next_display_id
from numbered
where cr.id = numbered.id
  and cr.display_id is null;

create sequence if not exists public.content_recordings_display_id_seq;

select setval(
  'public.content_recordings_display_id_seq',
  greatest(coalesce((select max(display_id) from public.content_recordings), 0), 1),
  coalesce((select max(display_id) from public.content_recordings), 0) > 0
);

alter sequence public.content_recordings_display_id_seq
  owned by public.content_recordings.display_id;

alter table if exists public.content_recordings
  alter column display_id set default nextval('public.content_recordings_display_id_seq'),
  alter column display_id set not null;

create unique index if not exists content_recordings_display_id_key
  on public.content_recordings (display_id);
