...
as $$
  ...
  ordered as (
    ...
    from grouped
    order by grouped.member_count desc, lower(grouped.group_name), grouped.group_name
...
as $$
  ...
  ordered as (
    ...
    order by lower(filtered_contacts.nama), filtered_contacts.nama, filtered_contacts.created_at desc
    limit greatest(10, least(100, coalesce(p_page_size, 20)))
    offset (greatest(coalesce(p_page, 1), 1) - 1) * greatest(10, least(100, coalesce(p_page_size, 20)))
  )
  select * from ordered;
