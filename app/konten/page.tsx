...
import { 
  Box, Typography, Container, Paper, Avatar, Tooltip, IconButton, Divider
} from "@mui/material";
...
export default function ContentManagement() {
  ...
  return (
    <Container maxWidth="md" sx={{ py: 6 }}>
      <Paper elevation={0} variant="outlined" sx={{ p: 4, borderRadius: 4, bgcolor: '#fafafa' }}>
          ...
          {platforms.map((p) => (
            <Tooltip title={p.name} key={p.id}>
              <IconButton 
                ...
                sx={{ 
                  transition: "0.3s",
                  transform: activePlatform === p.id ? "scale(1.2)" : "scale(1)",
                }}
              >
                <Avatar 
                  sx={{ 
                    ...
                    width: 56, 
                    height: 56,
                    boxShadow: activePlatform === p.id ? `0 0 15px ${p.color}66` : "none"
                  }}
                >
                  {p.icon}
                </Avatar>
        ...
        <Box sx={{ mt: 2 }}>
          ...
          {activePlatform === "instagram" && (
            <Box sx={{ textAlign: "center", py: 10 }}>
              ...
              <Typography color="text.secondary">Modul Instagram masih dalam pengembangan.</Typography>
            </Box>
          )}

          {activePlatform === "twitter" && (
            <Box sx={{ textAlign: "center", py: 10 }}>
              <X sx={{ fontSize: 60, color: "#e0e0e0", mb: 2 }} />
              <Typography color="text.secondary">Modul X (Twitter) masih dalam pengembangan.</Typography>
            </Box>
          )}
        </Box>
      </Paper>
