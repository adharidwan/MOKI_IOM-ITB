...
function matchesQuery(item: WhatsappOutboundListItem, query: string): boolean {
  if (!query) {
    return true;
  }
  ...
  const haystack = [
    ...
    item.instance_label || '',
    item.id,
    SOURCE_COPY[item.source_type]?.label || item.source_type,
  ]
    .join(' ')
    .toLowerCase();
      ...
      <Stack spacing={1}>
        ...
        <Stack spacing={0.35}>
          ...
          {item.last_delivery_error ? (
            <Typography sx={{ fontSize: '0.76rem', color: '#b91c1c' }}>
              {item.last_delivery_error}
            </Typography>
          ) : null}
        </Stack>
...
export default function OutboundTrackerOverlay() {
  ...
  useEffect(() => {
    const handleRegister = (event: Event) => {
      ...
      const ids = Array.isArray(customEvent.detail?.ids)
        ? customEvent.detail.ids.map((id) => String(id || '').trim()).filter(Boolean)
        : [];
      ...
      setTrackedIds((current) => Array.from(new Set([...current, ...ids])));
      setOpen(true);
    };
  ...
  }, [data?.items, query, sourceFilter, statusFilter]);
  ...
  const clearResolvedItems = () => {
    const resolvedIds = new Set(
      (data?.items || [])
        .filter((item) => item.delivery_status === 'sent' || item.delivery_status === 'failed')
        .map((item) => item.id),
    );

    setTrackedIds((current) => current.filter((id) => !resolvedIds.has(id)));
  };
  ...
  const hasTrackedItems = trackedIds.length > 0;
  ...
  return (
    <Box
      sx={{
        ...
        right: { xs: 12, md: 20 },
        bottom: { xs: 12, md: 20 },
        width: { xs: 'calc(100vw - 24px)', sm: open ? 380 : 380 },
      ...
      >
        ...
        >
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ alignItems: 'center'}} >
            ...
            />
            <Chip
              label={`${visibleSummary.sent} selesai`}
              size="small"
            ...
            <Chip
              ...
              size="small"
              sx={{ backgroundColor: '#eff6ff', color: '#1e40af', fontWeight: 700 }}
            />
        ...
        {open ? (
          <Stack spacing={2} sx={{ p: 2 }}>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
              <Chip label={`Ticket ${visibleSummary.queued_ticket_replies}`} size="small" variant="outlined" />
              <Chip label={`External ${visibleSummary.queued_api_notifications}`} size="small" variant="outlined" />
              <Chip label={`Blast ${visibleSummary.queued_blast_messages}`} size="small" variant="outlined" />
            </Stack>

            <TextField
              size="small"
              label="Cari nomor, ticket, referensi"
              value={query}
            ...
            {hasTrackedItems ? (
              <Stack spacing={1.25} sx={{ maxHeight: { xs: 320, md: 420 }, overflowY: 'auto', pr: 0.5 }}>
                ...
                {activeItems.length ? (
                  ...
                  <Typography sx={{ fontSize: '0.82rem', color: '#64748b' }}>
                    Tidak ada item aktif untuk filter ini.
                  </Typography>
                )}
                ...
                {failedItems.length ? (
                  ...
                  <Typography sx={{ fontSize: '0.82rem', color: '#64748b' }}>
                    Tidak ada item gagal untuk filter ini.
                  </Typography>
                )}
                ...
                {doneItems.length ? (
                  ...
                  <Typography sx={{ fontSize: '0.82rem', color: '#64748b' }}>
                    Belum ada item selesai untuk filter ini.
                  </Typography>
                )}
              </Stack>
            ) : (
